...
2. Gå tilbake til **`index.html`** — «Deres rute» bygges automatisk fra favorittene: kart med nummererte stopp i en fornuftig kjørerekkefølge fra Kongsberg. Ruten deles opp i **dagsetapper med ekte datoer** (avreise 6. juli, ~8 aktive timer per dag). Hver dag har en **🧭 Naviger etappen**-knapp som åpner hele dagens kjøretur i Google Maps med alle stoppene som viapunkter, og hver natt et **overnattingsforslag med ferdig utfylt Booking-søk** (riktig sted og datoer). Lang hjemreise får egen kjøredag. Hvert stopp har også en **🧭 Kart**-lenke og et **stort bilde** – trykk for **fullskjerm** og bla gjennom alle bildene (‹ ›, piltaster eller trykk på bildet). **🖨️ Skriv ut dagsplanen** gir en papirvennlig dag-for-dag-plan – grei å ha i hanskerommet der dekningen forsvinner.
...
6. **💾 Sikkerhetskopi / overfør:** knappen nederst i «Deres rute» lager en kode med alle valgene deres (rute, «nei»-bunke, bortvalgte og sjekkliste). Kopiér den til et notat som backup, eller last den ned som fil. Lim inn koden på en annen enhet og velg **🤝 Slå sammen** (beholder begge deler – fint når dere har swipet hver for dere) eller **♻️ Erstatt alt**. Ta gjerne en kopi før dere gjør store endringer!
